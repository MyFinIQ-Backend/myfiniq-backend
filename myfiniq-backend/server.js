...
console.warn("⚠️ STRIPE_SECRET_KEY fehlt.");
}
...
const BACKEND_URL =
process.env.BACKEND_URL || "https://myfiniq-backend.onrender.com";
const PORT = process.env.PORT || 10000;

console.log("🚀 MyFinIQ Backend startet...");
...
console.log("🔐 Firebase wird aus FIREBASE_SERVICE_ACCOUNT_JSON geladen...");
...
console.warn(
"⚠️ Weder FIREBASE_SERVICE_ACCOUNT_JSON noch serviceAccountKey.json gefunden. Firebase wird deaktiviert."
);
return null;
}

console.log("🔐 Firebase wird aus serviceAccountKey.json geladen...");
...
if (mailEnabled) {
console.log("✅ SMTP-Mailversand aktiviert");
} else {
console.log("⚠️ SMTP-Mailversand nicht vollständig eingerichtet");
}
...
res.status(500).json({
error:
"Firebase ist aktuell nicht verfügbar. Bitte FIREBASE_SERVICE_ACCOUNT_JSON oder serviceAccountKey.json prüfen."
});
return false;
}
return true;
...
if (!db) {
throw new Error("Firebase ist nicht verfügbar.");
}
...
result.push({
label: file.label,
url: `${BACKEND_URL.replace(/\/$/, "")}/api/download/${token}`
});
}
...
console.log("⚠️ E-Mail-Versand übersprungen: SMTP nicht vollständig eingerichtet.");
return;
}

if (!customerEmail) {
console.log("⚠️ E-Mail-Versand übersprungen: customerEmail fehlt.");
return;
}
...
html,
text:
`Vielen Dank für deinen Kauf bei MyFinIQ\n\n` +
`Hallo ${safeName},\n\n` +
...
try {
const { orderId, productId, customerEmail, customerName } = req.body;
...
if (!db) {
console.warn("⚠️ Webhook empfangen, aber Firebase ist nicht verfügbar.");
return res.json({ received: true, warning: "Firebase unavailable" });
}
...
if (metadata.type === "subscription" && metadata.uid && metadata.plan && db) {
await db.collection("users").doc(String(metadata.uid)).set(
{
plan: String(metadata.plan).toLowerCase(),
updatedAt: admin.firestore.FieldValue.serverTimestamp(),
stripeCheckoutCompleted: true,
